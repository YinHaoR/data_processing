...
#图片路径
ann_dir="E:\\dataset\\SSDD SAR dataset\\new"
saved_path="E:\\dataset\\SSDD SAR dataset\\new"
...
for ann in anns:
    ...
    sub.text = ann.split(".")[0]+".jpg"

    try:
        sub = root.find("path")
        sub.text =folder_path[0]+"/JPEGImages/"+ ann.split(".")[0] + ".jpg"
    except:
        newEle = ET.Element("path")  # 创建新节点并添加为root的子节点
        # newEle.attrib = {"name": "NewElement", "age": "20"}
        newEle.text = folder_path[0]+"/JPEGImages/"+ ann.split(".")[0] + ".jpg"
        root.append(newEle)
