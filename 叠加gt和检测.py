...
source_directory_img_path = 'F:\\NWPU-test\\test'
source_directory_label_path = 'F:\\test\\labels'
target_directory_path = 'F:\\test\\gt'
dic = {}
...
def draw_label(image_path, label_path):
    ...
    for _, x in enumerate(lb):
        ...
        # 绘图
        cv2.rectangle(img, (int(x[1]), int(x[2])), (int(x[3]), int(x[4])), (255, 255, 255),thickness=2)
        cv2.putText(img,None, (int(x[1]), int(x[2] - 2)), fontFace=cv2.FONT_HERSHEY_SIMPLEX, fontScale=1,
